...
/* 
Аэропорт
--> Jeez, это довольно сложная многопоточная реализация аэропорта, но, черт возьми,
она крутая. Ее принцип работы:

- Резервируется место под взлетно-посадочную полосу RUNWAY;
- Создаются последовательно объекты 3х самолетов;
- Во время создания в конструкторе запускаются потоки самолетов;
- Идет сложный метод:
    проверки занятости полосы объектом и занятия ее
    ожидания освобождения полосы
    взлета и освобождения полосы
- Все потоки работаю параллельно, но такая проверка позволяет создать рабочую очередь;
